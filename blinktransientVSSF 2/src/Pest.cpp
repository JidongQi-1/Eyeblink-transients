...
Pest::Pest(double startingLevel, 
		...
		if ( atoi(token) == 1+openInst )
		{
			...
			while (token != NULL)
			{
				switch ( pos )
				{
					...
					case 2: curLevel = atof(token); break;
					...
					case 9: isFixedStep = atoi(token); break;
...
double Pest::GetCurLevel()
{
	return( curLevel );
}

bool Pest::UpdateLevel(int isCorrect)
{
	hits += isCorrect;
	trials ++;
	
	//upper and lower bounds
	upB =  p * double(trials) + w ;
	loB =  p * double(trials) - w ;

	// step direction
	int dir = - int(hits >= upB) + int(hits <= loB) ;
	...
	if ( dir )
	{
		...
		if( !isFixedStep ){
			...
			else 
			{
				...
				if (  ( abs(stpCnt) == 3 && ( revPos < 2 || step[revPos-1] < step[revPos-2] ) ) ||
					( abs(stpCnt) > 3 )  )
				{
					if( isLog )
		...
		if (isLog) curLevel *= pow(step[s],dir);
		else curLevel += dir * step[s];
