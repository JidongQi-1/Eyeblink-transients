...
void main(void)
{
	...
	if(isWhiteNoise == 0){
		...
		vec4 color = texture(textureSampler, textureLocation);
		colorbs = noisyBitStealing(color.r, Rand( uint(gl_FragCoord.x), uint(gl_FragCoord.y), uint(rndseed1), uint(rndseed2) ) );
	}
	else{
		colorbs.r = float( Rand( uint(gl_FragCoord.x), uint(gl_FragCoord.y), uint(rndseed1), uint(rndseed2) ) > 0.5 );
		colorbs.g = colorbs.r;
		colorbs.b = colorbs.r;
	}
