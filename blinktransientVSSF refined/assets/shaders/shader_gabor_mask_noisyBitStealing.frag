...
uniform float xStimulus;	// Center of stimulus
uniform float yStimulus;	// Center of stimulus
uniform float xEnvelope;	// center of the envelope, e.g., the Gaussian envelope
uniform float yEnvelope;	// center of the envelope, e.g., the Gaussian envelope
//////////////////////////////////////////////////
...
uniform float degPerPix;	// degree/pixel
...
uniform int showMask;		// 0 not to show the mask, 1 to show a gabor mask, otherwise white noise mask
...
{
	// Get coordinate 
	float _xStim = gl_FragCoord.x - (screenOffsetX + xStimulus);
	float _yStim = gl_FragCoord.y - (screenOffsetY + yStimulus);
	float _xEnv = gl_FragCoord.x - (screenOffsetX + xEnvelope);
	float _yEnv = gl_FragCoord.y - (screenOffsetY + yEnvelope);
	
	// calculate sinusoidal grating or gabor
	float d2 = pow(_xEnv,2.0) + pow(_yEnv,2.0);
	float scale;
	if(showMask == 0)
		scale = amplitude * cos( PI_2 * spatialFreq * ( _xStim*sin(orientation) + _yStim*cos(orientation) ) + phase );
	else if(showMask == 1)	// plaid mask
		scale = 0.25 * cos( PI_2 *  1*degPerPix * ( _xStim*sin(orientation) + _yStim*cos(orientation) ) + phase ) + 0.25 * cos( PI_2 *  1*degPerPix * ( _xStim*sin(orientation+PI_2/4) + _yStim*cos(orientation+PI_2/4) ) + phase ) +
				0.25 * cos( PI_2 * 10*degPerPix * ( _xStim*sin(orientation) + _yStim*cos(orientation) ) + phase ) + 0.25 * cos( PI_2 * 10*degPerPix * ( _xStim*sin(orientation+PI_2/4) + _yStim*cos(orientation+PI_2/4) ) + phase );
	else if(showMask == 2)	// white noise mask
		scale = float( Rand( uint(_xStim+960), uint(_yStim+540), uint(rndseed1), uint(rndseed2) ) > 0.5 ) * 2 - 1;
