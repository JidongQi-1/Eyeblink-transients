...
{
	// Get coordinate 
	float x = (gl_FragCoord.x - (screenOffsetX + cX)) * degPerPix;
	float y = (gl_FragCoord.y - (screenOffsetY + cY)) * degPerPix;

	// calculate sinusoidal grating
	float scale = contrast * cos(2*3.14159*spatialFreq * (x*cos(orientation) + y*sin(orientation))) / 2.0 + 0.5;
